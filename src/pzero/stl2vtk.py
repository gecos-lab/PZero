...
def vtk2stl(self=None, out_dir_name=None):
    ...
    for uid in self.geol_coll._df['uid']:
        if isinstance(self.geol_coll.get_uid_vtk_obj(uid), TriSurf):
            header = (uid + "_" + self.geol_coll._df.loc[self.geol_coll._df['uid'] == uid, 'name'].values[0])
    ...
    for uid in self.boundary_coll._df['uid']:
        if isinstance(self.boundary_coll.get_uid_vtk_obj(uid), TriSurf):
            header = (uid + "_" + self.boundary_coll._df.loc[self.boundary_coll._df['uid'] == uid, 'name'].values[0])
