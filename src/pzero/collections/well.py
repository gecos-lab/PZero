...
class WellCollection(Collection):
    ...
    def add_entity_from_dict(self, entity_dict=None, color=None):
        ...
        if self.main_window.well_legend_df.loc[self.main_window.well_legend_df['Loc ID'] == locid].empty:
            R, G, B = np_round(np_random.random(3) * 255)
            self.main_window.well_legend_df = self.main_window.well_legend_df.append({'Loc ID': locid,
                                                                            'color_R': R,
                                                                            'color_G': G,
                                                                            'color_B': B,
                                                                            'line_thick': 2.0,
            ...
            self.main_window.prop_legend.update_widget(self.main_window)
        """Then emit signal to update the views."""
        self.main_window.well_added_signal.emit(
    ...
    def remove_entity(self, uid=None):
        ...
        self.main_window.well_removed_signal.emit([uid])  # a list of uids is emitted, even if the entity is just one
        return uid
    ...
    def append_uid_property(self, uid=None, property_name=None, property_components=None):
        ...
        # self.main_window.prop_legend.update_widget(self.main_window)
        self.main_window.well_metadata_modified_signal.emit([uid])
    ...
    def remove_uid_property(self, uid=None, property_name=None):
        ...
        """IN THE FUTURE add cell data"""
        # self.main_window.prop_legend.update_widget(self.main_window)
        self.main_window.well_data_keys_removed_signal.emit([uid])
