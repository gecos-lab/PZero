...
from pzero.point_clouds import (
    normals2dd,
    cut_pc,
    decimate_pc,
    segment_pc,
...
def _make_self(
    vtk_obj, uid: str = "uid_001", dip_data: bool = True, name: str = "Pc"
) -> MagicMock:
    """
    ...
    if dip_data:
        self_mock.parent.dom_coll.get_uid_properties_names.return_value = [
            "dip",
            "dip direction",
        ]
...
def _make_self_multi(vtk_objs: list, uids: list) -> MagicMock:
    ...
    self_mock.parent.dom_coll.get_uid_vtk_obj.side_effect = lambda uid: vtk_objs[
        uids.index(uid)
    ]
    self_mock.parent.dom_coll.get_uid_properties_names.return_value = ["Normals"]
    return self_mock
...
class TestNormals2dd:
    ...
    def test_dip_values_in_valid_range(self):
        ...
        assert np.all(dip >= 0) and np.all(
            dip <= 90
...
class TestDecimatePC:
    ...
    def test_decimated_pc_size(self):
        ...
        assert (
            decimated_pc.GetNumberOfPoints() != 0
            and decimated_pc.GetNumberOfPoints() is not None
        ), "Problem, size is 0"
        np.testing.assert_allclose(
            decimated_pc.GetNumberOfPoints(),
            vtk_obj.GetNumberOfPoints() * 0.5,
            atol=1,
...
class TestSegmentPC:
    ...
    def test_data_outside_threshold(self, mock_dialog):
        ...
        assert (
            seg_pc.GetNumberOfPoints() != 0 and seg_pc.GetNumberOfPoints() is not None
        ), "Problem, size is 0"
        # The pc size should be 200 since there is only one cluster left
        assert (
            180 <= seg_pc.GetNumberOfPoints() <= 200
        ), "Problem, there is no one cluster left"
    ...
    def test_with_real_dat_file(self, request, mock_dialog):
        ...
        assert np.all(dip >= mock_dialog.return_value["d1"]) and np.all(
            dip <= mock_dialog.return_value["d2"]
        )
        assert np.all(dip_dir >= mock_dialog.return_value["dd1"]) and np.all(
            dip_dir <= mock_dialog.return_value["dd2"]
        )
        for uid in np.unique(cluster_ids):
            cluster_size = np.sum(cluster_ids == uid)
            assert (
                cluster_size > mock_dialog.return_value["nn"]
...
class TestFacetsPC:
    ...
    def test_n_poly(self, mock_dialog):
        ...
        assert (
            len(facets.get_field_data("Normals")) // 3 == n_clust
        ), "There is a wrong number of normals"
        assert (
            len(facets.get_field_data("Centers")) // 3 == n_clust
        ), "There is a wrong number of centers"
        assert (
            len(facets.get_field_data("dip")) == n_clust
        ), "There is a wrong number of dip"
        assert (
            len(facets.get_field_data("dip direction")) == n_clust
        ), "There is a wrong number of dir"
        assert (
            len(facets.get_field_data("area")) == n_clust
        ), "There is a wrong number of area"
        assert (
            len(facets.get_field_data("width")) == n_clust
        ), "There is a wrong number of width"
        assert (
            len(facets.get_field_data("length")) == n_clust
        ), "There is a wrong number of length"
    ...
    def test_positive_width_length(self, mock_dialog):
        ...
        facets = entity_dict_facets["vtk_obj"]
        assert np.all(
            facets.get_field_data("width") > 0
        ), "Some width are non positives"
        assert np.all(
            facets.get_field_data("length") > 0
        ), "Some length are non positives"
    ...
    def test_with_real_dat_file(self, request, mock_dialog):
        ...
        assert np.all(dip_dir >= 0) and np.all(
            dip_dir < 360
        ), f"Dip direction out of [0, 360): min={dip_dir.min():.1f}  max={dip_dir.max():.1f}"

        # Area is positive for every facet
        area = facets.get_field_data("area")
        assert np.all(area > 0), f"Some facets have zero or negative area: {area}"
        ...
        norms = np.linalg.norm(normals, axis=1)
        np.testing.assert_allclose(
            norms, 1.0, atol=1e-5, err_msg="Normals are not unit vectors"
        )
...
class TestCalibrationPC:
    ...
    def mock_plot(self):
        ...
        """
        with patch("pzero.point_clouds.plt_subplot") as mock_subplot, patch(
            "pzero.point_clouds.sns"
        ) as mock_sns:
            mock_fig = MagicMock()
    ...
    def test_distance(self):
        ...
        assert (
            distances1 is not None and distances2 is not None and distances3 is not None
        ), "One of the distances is None"

        assert (
            np.all(distances1 >= 0)
            and np.all(distances2 >= 0)
            and np.all(distances3 >= 0)
        ), "One of the distances is not positives"

        assert not np.array_equal(
            distances1, distances2
        ), "pc1 and pc2 have identical distances"
        assert not np.array_equal(
            distances1, distances3
        ), "pc1 and pc3 have identical distances"

        assert (
            self_mock.parent.dom_coll.replace_vtk.call_count == 3
        ), "All the vtk did not got updated"
...
class TestAutoPick:
    ...
    def test_feature_name_is_right(self, mock_dialog):
        ...
        assert (
            name == self_mock_pick.parent.dom_coll.get_uid_name.return_value
        ), "The feature name does not corresponding."
    ...
    def test_Attitude_has_right_number_of_points(self, mock_dialog):
        ...
        assert (
            0 < n_attitude_points <= n_clust
        ), "There is not the right number of pick points."
    ...
    def test_with_real_dat_file(self, request, mock_dialog):
        ...
        assert (
            entity_dict["feature"]
            == self_mock_ap.parent.dom_coll.get_uid_name.return_value
...
class TestThreshFilter:
    ...
    def test_data_out_thresh(self, mock_dialog):
        ...
        assert (
            pc_filt.GetNumberOfPoints() != 0 and pc_filt.GetNumberOfPoints() is not None
        ), "Problem, size is 0"
        # The pc size should be 400 since there is only two cluster left
        assert (
            380 <= pc_filt.GetNumberOfPoints() <= 400
        ), "Problem, there is no two clusters left"
    ...
    def test_new_arrays_right(self, mock_dialog):
        ...
        for key in vtk_obj.point_data_keys:
            assert (
                key in pc_filt.point_data_keys
            ), f"Property '{key}' missing from result"
