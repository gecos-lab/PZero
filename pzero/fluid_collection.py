...
class FluidsCollection(QAbstractTableModel):
    ...
    def remove_entity(self, uid=None):
        ...
        if not uid in self.get_uids():
            return
        self.df.drop(self.parent.fluid_coll.df[self.parent.fluid_coll.df['uid'] == uid].index, inplace=True)
        ...
        for fluid_type in fluid_types_in_legend:
            if self.parent.fluid_coll.df.loc[self.parent.fluid_coll.df['fluid_type'] == fluid_type].empty:
            ...
            for feature in features_in_legend:
                if self.parent.fluid_coll.df.loc[(self.parent.fluid_coll.df['fluid_type'] == fluid_type) & (self.parent.fluid_coll.df['fluid_feature'] == feature)].empty:
                    """Get index of row to be removed, then remove it in place with .drop()."""
                ...
                for scenario in scenarios_in_legend:
                    if self.parent.fluid_coll.df.loc[(self.parent.fluid_coll.df['fluid_type'] == fluid_type) & (self.parent.fluid_coll.df['fluid_feature'] == feature) & (self.parent.fluid_coll.df['scenario'] == scenario)].empty:
                        """Get index of row to be removed, then remove it in place with .drop()."""
                        ...
                        table_updated = table_updated or True
        for feature in features_in_legend:
            if self.parent.fluid_coll.df.loc[self.parent.fluid_coll.df['fluid_feature'] == feature].empty:
                """Get index of row to be removed, then remove it in place with .drop()."""
                idx_remove = self.parent.fluids_legend_df[self.parent.fluids_legend_df['fluid_feature'] == feature].index
                self.parent.fluids_legend_df.drop(idx_remove, inplace=True)
                table_updated = table_updated or True
            for scenario in scenarios_in_legend:
                if self.parent.fluid_coll.df.loc[(self.parent.fluid_coll.df['fluid_feature'] == feature) & (self.parent.fluid_coll.df['scenario'] == scenario)].empty:
                    """Get index of row to be removed, then remove it in place with .drop()."""
                    ...
                    table_updated = table_updated or True
        for scenario in scenarios_in_legend:
            if self.parent.fluid_coll.df.loc[self.parent.fluid_coll.df['scenario'] == scenario].empty:
                """Get index of row to be removed, then remove it in place with .drop()."""
    ...
    def fluid_attr_modified_update_legend_table(self):
        ...
        for uid in self.parent.fluid_coll.df['uid'].to_list():
            fluid_type = self.parent.fluid_coll.df.loc[self.parent.fluid_coll.df['uid'] == uid, "fluid_type"].values[0]
            feature = self.parent.fluid_coll.df.loc[self.parent.fluid_coll.df['uid'] == uid, "fluid_feature"].values[0]
            scenario = self.parent.fluid_coll.df.loc[self.parent.fluid_coll.df['uid'] == uid, "scenario"].values[0]
            if self.parent.fluids_legend_df.loc[(self.parent.fluids_legend_df['fluid_type'] == fluid_type) & (self.parent.fluids_legend_df['fluid_feature'] == feature) & (self.parent.fluids_legend_df['scenario'] == scenario)].empty:
                self.parent.fluids_legend_df = self.parent.fluids_legend_df.append({'fluid_type': fluid_type,
                                                                                'fluid_feature': feature,
                                                                                'scenario': scenario,
