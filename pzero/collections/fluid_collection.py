...
class FluidsCollection(BaseCollection):
    ...
    def add_entity_from_dict(self, entity_dict: pd_DataFrame = None, color: np_ndarray = None):
        ...
        self.parent.fluid_coll.signals.added.emit(
            [entity_dict["uid"]]
        )
    ...
    def remove_entity(self, uid: str = None) -> str:
        ...
        self.parent.fluid_coll.signals.removed.emit([uid])
        return uid
    ...
    def replace_vtk(self, uid: str = None, vtk_object: vtkDataObject = None):
        ...
        if isinstance(vtk_object, type(self.df.loc[self.df["uid"] == uid, "vtk_obj"].values[0])):
            ...
            self.parent.prop_legend.update_widget(self.parent)
            self.parent.fluid_coll.signals.data_keys_modified.emit([uid])
            self.parent.fluid_coll.signals.geom_modified.emit([uid])
        else:
    ...
    #     self.parent.fluid_coll.signals.metadata_modified.emit(updated_list)

    # def data_keys_modified_signal(self, updated_list: list = None):
    #     """Signal emitted when point data keys change."""
    #     self.parent.fluid_coll.signals.data_keys_modified.emit(updated_list)
