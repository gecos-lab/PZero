...
from numpy import set_printoptions as np_set_print_options, ndarray
...
class DomCollection(BaseCollection):
    ...
    def add_entity_from_dict(self, entity_dict: DataFrame = None, color: ndarray = None):
        ...
        self.parent.dom_added_signal.emit(
            [entity_dict["uid"]]
    ...
    def remove_entity(self, uid: str = None) -> str:
        ...
        self.parent.dom_removed_signal.emit(
            [uid]
        )  # a list of uids is emitted, even if the entity is just one
        return uid
...
#     }
#
#     """List of valid data types.""" # this should be topo while type should not be here
...
#         self.df = pd_DataFrame(columns=list(self.dom_entity_dict.keys()))
...
#                 self.parent.dom_metadata_modified_signal.emit(
#                     [uid]
#                 )  # a list of uids is emitted, even if the entity is just one
#                 return True
#         return QVariant()
