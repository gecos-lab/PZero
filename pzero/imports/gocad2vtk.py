    ...
    for line in fin:
        ...
        if clean_line[0] == "GOCAD":
            ...
            if clean_line[1] == "VSet":
                ...
                curr_obj_dict["type"] = type_default 
            ...
            curr_obj_dict["type"] = ("_".join(clean_line[1:])).lower()
            if (
                curr_obj_dict["type"]
                not in self.geol_coll.valid_types
            ):
                if "Fault" in curr_obj_dict["type"]:
                    curr_obj_dict["type"] = "fault"
                elif "fault" in curr_obj_dict["type"]:
                    curr_obj_dict["type"] = "fault"
                elif "Horizon" in curr_obj_dict["type"]:
                    curr_obj_dict["type"] = "top"
                else:
                    curr_obj_dict["type"] = "undef"
...
def gocad2vtk_boundary(self=None, in_file_name=None, uid_from_name=None):
    ...
    for line in fin:
        ...
        # elif clean_line[0] == 'GEOLOGICAL_TYPE':
        #     curr_obj_dict['type'] = ("_".join(clean_line[1:])).lower()
        #     if curr_obj_dict['type'] not in self.geol_coll.valid_types:
        #         if "Fault" in curr_obj_dict['type']:
        #             curr_obj_dict['type'] = "fault"
        #         elif "fault" in curr_obj_dict['type']:
        #             curr_obj_dict['type'] = "fault"
...
def vtk2gocad(self=None, out_file_name=None):
    ...
    for uid in self.geol_coll.df["uid"].to_list():
        ...
        if topology in [
            ...
            type = self.geol_coll.df.loc[
                self.geol_coll.df["uid"] == uid, "type"
            ].values[0]
            ...
            fout.write("GEOLOGICAL_TYPE " + type + "\n")
