...
class BaseView(QDockWidget, Ui_QDockWidget):
    ...
    def update_fluids_tree_removed(
        self, removed_list=None
    ):  # second attchild_fluid_featempt
    ...
    def closeEvent(self, event):
        ...
        if reply == QMessageBox.Yes:
            # Disconnect all the signals - needed with DeleteOnClose Window Attribute
...
class View2D(BaseView):
    ...
    def initialize_interactor(self):
        ...
        self.ax = (
            self.figure.gca()
        )  # create reference to plt figure axes gca() = "get current axes"
        ...
        self.ax.set_aspect(
            1.0
        )  # set axis aspect ratio such as height is 1 times the width. aspect=1 is the same as aspect=’equal’. vertical exaggeration is 1
        ...
        self.text_msg = self.ax.add_artist(
            TextArea(
                "some text",
                textprops=dict(fontsize=int(self.base_font_size), color="crimson"),
            )
        )
        self.text_msg.set_offset(
            (int(self.base_font_size * 0.4), int(self.base_font_size * 0.4))
        )
    ...
    def select_actor_with_mouse(self):
        ...
        def select_actor_pick(event):
            while self.selected_uids == []:
                if event.artist:
                    if (
                        self.actors_df.loc[
                            self.actors_df["actor"] == event.artist, "collection"
                        ].values[0]
                        == "geol_coll"
                    ):
                        """IN THE FUTURE check why the condition above rises an error, but then the code runs flawlessly."""
                        uid = self.actors_df.loc[
                            self.actors_df["actor"] == event.artist, "uid"
                        ].values[0]
                        if (
                            self.actors_df.loc[
                                self.actors_df["uid"] == uid, "show"
        ...
        self.cid_select = self.canvas.mpl_connect(
            "pick_event", select_actor_pick
        )  # connects to callback function. cid is just an integer id
    ...
    def pick_with_mouse(self):
        ...
        def pwm_press_callback(event):
            ...
            try:
                ...
                self.text_msg.set_text(
                    "x: {0:.2f} y: {1:.2f} x: {2:d} y: {3:d} btn: {4:d}".format(
                        self.pick_with_mouse_U_data,
        ...
        self.canvas.setFocusPolicy(Qt.ClickFocus)
        self.canvas.setFocus()
        self.cid_pwm_1 = self.canvas.mpl_connect(
            "button_press_event", pwm_press_callback
        )
        self.cid_pwm_2 = self.canvas.mpl_connect(
            "motion_notify_event", pwm_motion_callback
        )
        self.cid_pwm_3 = self.canvas.mpl_connect(
            "button_release_event", pwm_release_callback
        )
    ...
    def vector_by_mouse(self, verbose=False):
        ...
        def vbm_motion_callback(event):
            ...
            if event.button == 1:  # Do this only if left button is pressed (= 1)
                try:
                    ...
                    self.temp_vbm_length = sqrt(
                        self.temp_vbm_dU**2 + self.temp_vbm_dV**2
                    )
                    self.temp_vbm_azimuth = degrees(
                        atan2(self.temp_vbm_dU, self.temp_vbm_dV)
                    )
                    # self.temp_vbm_line.set_data([self.temp_vbm_U0, event.xdata], [self.temp_vbm_V0, event.ydata])
                    self.temp_vbm_line.set_data(
                        [self.temp_vbm_U0, self.temp_vbm_dU + self.temp_vbm_U0],
            ...
            if event.button == 1:  # Do this only if left button is pressed (= 1)
                try:
                    ...
                    if (
                        ...
                        self.vector_by_mouse_azimuth = (
                            self.vector_by_mouse_azimuth + 360
                        )
                    ...
                    self.text_msg.set_text(
                        "dU: {0:.2f} dV: {1:.2f} length: {2:.2f} azimuth: {3:.2f}".format(
                            self.vector_by_mouse_dU,
                            self.vector_by_mouse_dV,
                            self.vector_by_mouse_length,
                            self.vector_by_mouse_azimuth,
        ...
        self.cid_vbm_1 = self.canvas.mpl_connect(
            "button_press_event", vbm_press_callback
        )
        self.cid_vbm_2 = self.canvas.mpl_connect(
            "motion_notify_event", vbm_motion_callback
        )
        self.cid_vbm_3 = self.canvas.mpl_connect(
            "button_release_event", vbm_release_callback
        )
        self.figure.canvas.start_event_loop(timeout=-1)
...
class ViewMap(View2D):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.ax.set_title(
            "Map", fontsize=self.base_font_size
        )  # set title _____________________________________________________UPDATE
        self.ax.set_xlabel(
            "X [m]", fontsize=int(self.base_font_size * 0.8)
        )  # set label for W coordinate
        self.ax.set_ylabel(
            "Y [m]", fontsize=int(self.base_font_size * 0.8)
        )  # set label for Z coordinate
    ...
    def initialize_menu_tools(self):
        ...
        self.sectionFromAzimuthButton.triggered.connect(
            lambda: section_from_azimuth(self)
        )  # connect action to function with additional argument parent
        self.menuBaseView.addAction(self.sectionFromAzimuthButton)  # add action to menu

        self.sectionFromPointsButton = QAction(
            "Section from 2 points", self
        )  # create action
        ...
        self.boundaryFromPointsButton.triggered.connect(
            lambda: boundary_from_points(self)
        )  # connect action to function with additional argument parent
    ...
    def show_actor_with_property(
        ...
        """Then plot."""
        if isinstance(
            plot_entity, (VertexSet, PolyLine, XsVertexSet, XsPolyLine, Attitude)
        ):
            if isinstance(plot_entity.points, np_ndarray):
                    ...
                    if isinstance(plot_entity, (VertexSet, Attitude)):
                        if uid in self.selected_uids:
                            if show_property == "Normals":
                                U = np_sin(
                                    (plot_entity.points_map_dip_azimuth + 90)
                                    * np_pi
                                    / 180
                                )
                                V = np_cos(
                ...
                if (plot_entity.bounds[0] != plot_entity.bounds[1]) and (
                    ...
                    if show_property not in [None, "none"]:
                        if plot_entity.get_property_components(show_property) == 3:
                            ...
                            this_actor = self.ax.imshow(
                                plot_entity.image_data(show_property),
                                origin="upper",
                                extent=xy_bounds,
                                zorder=0,
                            )
...
class ViewXsection(View2D):
    ...
    def __init__(self, parent=None, *args, **kwargs):
        ...
        self.ax.set_title(
            ("Xsection: " + self.this_x_section_name), fontsize=self.base_font_size
        )  # set title
        self.ax.set_xlabel(
            "W [m]", fontsize=int(self.base_font_size * 0.8)
        )  # set label for W coordinate
        self.ax.set_ylabel(
            "Z [m]", fontsize=int(self.base_font_size * 0.8)
        )  # set label for Z coordinate
    ...
    def show_actor_with_property(
        ...
        if collection == "geol_coll":
            if (
                self.parent.geol_coll.get_uid_topological_type(uid) == "XsVertexSet"
                or self.parent.geol_coll.get_uid_topological_type(uid) == "XsPolyLine"
            ) and self.parent.geol_coll.get_uid_x_section(
                uid
            ...
            if (
                self.parent.boundary_coll.get_uid_topological_type(uid) == "XsPolyLine"
                and self.parent.boundary_coll.get_uid_x_section(uid)
                == self.this_x_section_uid
            ):
                color_R = self.parent.boundary_coll.get_legend()["color_R"]
                color_G = self.parent.boundary_coll.get_legend()["color_G"]
            ...
            if (
                self.parent.mesh3d_coll.get_uid_mesh3d_type(uid) == "XsVoxet"
                and self.parent.mesh3d_coll.get_uid_x_section(uid)
                == self.this_x_section_uid
            ):
                color_R = self.parent.mesh3d_coll.get_legend()["color_R"]
                color_G = self.parent.mesh3d_coll.get_legend()["color_G"]
            ...
            if (
                self.parent.dom_coll.get_uid_dom_type(uid) == "DomXs"
                and self.parent.dom_coll.get_uid_x_section(uid)
                == self.this_x_section_uid
            ):
                color_R = self.parent.dom_coll.get_legend()["color_R"]
                color_G = self.parent.dom_coll.get_legend()["color_G"]
            ...
            if (
                self.parent.image_coll.get_uid_image_type(uid) == "XsImage"
                and self.parent.image_coll.get_uid_x_section(uid)
                == self.this_x_section_uid
            ):
                color_R = self.parent.image_coll.get_legend()["color_R"]
                color_G = self.parent.image_coll.get_legend()["color_G"]
        ...
        if plot_entity:
            if isinstance(plot_entity, XsVoxet):
                    ...
                    if (plot_entity.bounds[0] != plot_entity.bounds[1]) and (
                        ...
                        if show_property not in [None, "none"]:
                            ...
                            (
                                ...
                                self.ax.get_xlim()
                            )  # needed since sometimes plotting an image resizes the plot to the image area only
                    ...
                    if (plot_entity.bounds[0] != plot_entity.bounds[1]) and (
                        ...
                        if show_property not in [None, "none"]:
                            ...
                            if plot_entity.get_property_components(show_property) == 3:
                                ...
                                plot_entity.get_property_components(show_property) == 1
                            ):
                                """Greyscale for single property images"""
                                show_property_title = show_property
                                show_property_cmap = self.parent.prop_legend_df.loc[
                ...
                isinstance(plot_entity, (Well, WellTrace, WellMarker))
                or collection == "backgrounds_coll"
            ):
                this_actor = None
            else:
                if isinstance(plot_entity.points, np_ndarray):
                    if plot_entity.points_number > 0:
                        ...
                        if isinstance(plot_entity, XsVertexSet):
                            if uid in self.selected_uids:
                                if show_property == "Normals":
                                    U = np_cos(
                                        plot_entity.points_xs_app_dip * np_pi / 180
                                    )
                                    V = np_sin(
                                        plot_entity.points_xs_app_dip * np_pi / 180
                                    )
