...
class PCDom(PolyData):  # _______________________ DO WE NEED ADDITIONAL METHODS WITH RESPECT TO POLYDATA?
    """Point Cloud DOM.
    See discussion at https://discourse.vtk.org/t/proposal-adding-a-vtkpointcloud-data-structure/3872/3-> pointset would be better since no cell data is necessary but there are some problems (related to VTK and pyvista):

    1. Many functions that could be useful to filter and elaborate the pc need celldata (e.g. vktthreshehold).
    2. Many alternative functions designed for point clouds (with no cell data) usually are not very optimized or miss some important features (e.g. vtkPointThreshold does not have an InverOn option)
    3. I could not find a way to save pointsets without casting to polydata and use vtkxmlPolyDataWriter.
    4. For now pyvista has a problem visualizing vtkpointsets (not pyvista.PointSet). Now this problem is resolved but we need to wait for an official release. To circumvent this problem it's necessary to cast vtkPointSet to pyvista.PointSet() (not the best)
    ...
    @property
    def points_map_dip_azimuth(self):
        """Returns dip azimuth (in grad) as Numpy array for map plotting if points have Normals property. Must be redefined since we use negative z vectors"""
        if "Normals" in self.point_data_keys:
            map_dip_azimuth = np_arctan2(self.get_point_data("Normals")[:, 0], self.get_point_data("Normals")[:, 1]) * 180 / np_pi + 180
