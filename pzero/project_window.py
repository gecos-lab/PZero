...
from vtk import vtkPolyData, vtkAppendPolyData, vtkOctreePointLocator, vtkXMLPolyDataWriter, \
    vtkXMLStructuredGridWriter, vtkXMLImageDataWriter, vtkXMLStructuredGridReader, \
    vtkXMLPolyDataReader, vtkXMLImageDataReader
...
from pzero.entities_factory import VertexSet, PolyLine, TriSurf, XsVertexSet, XsPolyLine, \
    DEM, MapImage, Voxet, Seismics, XsVoxet, PCDom, TSDom, Well, Attitude, XsImage
...
from pzero.helpers.helper_dialogs import options_dialog, save_file_dialog, open_file_dialog, \
    input_combo_dialog, message_dialog, multiple_input_dialog, input_one_value_dialog, \
    progress_dialog, import_dialog, PreviewWidget
...
from pzero.three_d_surfaces import interpolation_delaunay_2d, poisson_interpolation, \
    implicit_model_loop_structural, surface_smoothing, linear_extrusion, decimation_pro_resampling, \
    decimation_quadric_resampling, subdivision_resampling, intersection_xs, project_2_dem, \
    project_2_xs, split_surf, retopo
...
class ProjectWindow(QMainWindow, Ui_ProjectWindow):
    ...
    """Signals defined here are meant to be broadcast TO ALL views. They generally pass a list of uid's."""
    project_close_signal = Signal()  # used to close open windows when the current project is closed

    # signals for collections could be moved to collection classes <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.actionShowInfo.triggered.connect(self.show_vtk_info)
        ...
        """The update_actors property is switched on and off to trigger auto-update of actros in views when editing.
        Default behaviour is auto-update ON = True."""
    ...
    def closeEvent(self, event):
        ...
        if reply == QMessageBox.Yes:
            ...
            # The following was an alternative to clode dock windows
    ...
    def entity_remove(self):
        ...
        if not self.selected_uids:
            self.TextTerminal.appendPlainText("nothing selected")
            return
        ...
        for uid in self.selected_uids:
            ...
            self.TextTerminal.appendPlainText("removed entity with uid: " + uid)
        self.update_actors = True

    def entities_merge(
            self):  # CHECK (1) HOW PROPERTIES AND TEXTURES ARE AFFECTED BY MERGING, (2) HOW IT WORKS FOR DOMs <<<<<<<<<
        ...
        self.update_actors = True
        self.TextTerminal.appendPlainText("merge entity created with uid: " + uid_new)
    ...
    def texture_add(self):
        ...
        if not self.shown_table == "tabDOMs":
            self.TextTerminal.appendPlainText("no DOM selected")
            return
        if not self.selected_uids:
            self.TextTerminal.appendPlainText("no DOM selected")
            return
        ...
        for dom_uid in dom_uids:
            if isinstance(self.dom_coll.get_uid_vtk_obj(dom_uid), DEM):
                ...
                self.TextTerminal.appendPlainText(
                    "texture with uid: " + map_image_uid + " added to dom with uid: " + dom_uid)
        self.update_actors = True
    ...
    def texture_remove(self):
        ...
        """Map Image selection dialog."""
        map_image_names = self.image_coll.df.loc[self.image_coll.df['image_type'] == "MapImage", 'name'].to_list()
        map_image_name = input_combo_dialog(parent=None,
                                            title="Remove texture from DEM",
                                            label="Choose Map Image",
                                            choice_list=map_image_names)
        if not map_image_name:
            self.TextTerminal.appendPlainText("no texture selected")
            return
        map_image_uid = self.image_coll.df.loc[self.image_coll.df['name'] == map_image_name, 'uid'].values[0]
        if map_image_uid not in self.image_coll.get_uids():
            self.TextTerminal.appendPlainText("no texture selected")
        ...
        if map_image_uid in self.image_coll.get_uids():
            ...
            for dom_uid in dom_uids:
                ...
                self.TextTerminal.appendPlainText(
                    "texture with uid: " + map_image_uid + " removed from dom with uid: " + dom_uid)
        self.update_actors = True
        ...
        if self.shown_table == "tabGeology":
            for uid in self.selected_uids:
                if not updt_dict['property_name'] in self.geol_coll.get_uid_properties_names(uid):
                    self.geol_coll.append_uid_property(uid=uid, property_name=updt_dict['property_name'],
            ...
            for uid in self.selected_uids:
                if not updt_dict['property_name'] in self.mesh3d_coll.get_uid_properties_names(uid):
                    self.mesh3d_coll.append_uid_property(uid=uid, property_name=updt_dict['property_name'],
                                                         property_components=updt_dict['property_components'])
        elif self.shown_table == "tabDOMs":
            for uid in self.selected_uids:
                if not updt_dict['property_name'] in self.dom_coll.get_uid_properties_names(uid):
                    self.dom_coll.append_uid_property(uid=uid, property_name=updt_dict['property_name'],
                                                      property_components=updt_dict['property_components'])
    ...
    def property_remove(self):  # ____________________________________________________ ADD IMAGES
        ...
        if self.shown_table == "tabGeology":
            ...
            if len(self.selected_uids) > 1:
                for uid in self.selected_uids[1:]:
                    property_name_list = list(
                        set(property_name_list) & set(self.mesh3d_coll.get_uid_properties_names(uid=uid)))
            if property_name_list == []:
                return
            property_name = input_combo_dialog(parent=None, title="Remove selected property", label="Remove property",
                                               choice_list=property_name_list)
            ...
            if len(self.selected_uids) > 1:
                for uid in self.selected_uids[1:]:
                    property_name_list = list(
                        set(property_name_list) & set(self.dom_coll.get_uid_properties_names(uid=uid)))
            if property_name_list == []:
                return
            property_name = input_combo_dialog(parent=None, title="Remove selected property", label="Remove property",
                                               choice_list=property_name_list)
    ...
    def normals_calculate(
        ...
        set_normals(self)
        self.update_actors = True
        self.TextTerminal.appendPlainText("normal calculated on selected entities")
    ...
    def show_vtk_info(self):
        """Show info about selected VTK entitis in terminal."""
        if self.selected_uids:
            for uid in self.selected_uids:
                if self.shown_table == "tabGeology":
                    ...
                    name = self.xsect_coll.get_uid_name(uid)
                    entity = self.xsect_coll.get_uid_vtk_obj(uid)
                elif self.shown_table == "tabMeshes3D":
                    name = self.mesh3d_coll.get_uid_name(uid)
                    entity = self.mesh3d_coll.get_uid_vtk_obj(uid)
                elif self.shown_table == "tabDOMs":
                    name = self.dom_coll.get_uid_name(uid)
                    entity = self.dom_coll.get_uid_vtk_obj(uid)
                elif self.shown_table == "tabImages":
                    name = self.image_coll.get_uid_name(uid)
                    entity = self.image_coll.get_uid_vtk_obj(uid)
                elif self.shown_table == "tabBoundaries":
                    name = self.boundary_coll.get_uid_name(uid)
                    entity = self.boundary_coll.get_uid_vtk_obj(uid)
                elif self.shown_table == "tabWells":
                    name = self.well_coll.get_uid_name(uid)
                    entity = self.well_coll.get_uid_vtk_obj(uid)
                self.TextTerminal.appendPlainText("name: " + name + "\nentity: " + str(entity))

    def build_octree(self):
        """ADD COMMENTS, self.update_actors = False/True and terminal output<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<"""
        if self.selected_uids:
            for uid in self.selected_uids:
                if self.shown_table == "tabGeology":
                    entity = self.geol_coll.get_uid_vtk_obj(uid)
            ...
            for uid in self.selected_uids:
                if self.shown_table == "tabDOMs":
                    ...
                    vtk_out_dict = deepcopy(
                        collection.df.loc[collection.df['uid'] == uid].drop(['uid', 'vtk_obj'], axis=1).to_dict(
                            'records')[0])
    ...
    def smooth_dialog(self):
        """ADD COMMENTS, self.update_actors = False/True and terminal output<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<"""
        # <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< TO BE CHECKED - PreviewWidget shows only last entity?
    ...
    def connected_parts(self):
        ...
        for uid in self.selected_uids:
            if isinstance(collection.get_uid_vtk_obj(uid), (PolyLine, TriSurf)):
                ...
                collection.get_uid_vtk_obj(uid).connected_calc()
            self.TextTerminal.appendPlainText("connected parts calculated for uid: " + uid)
        self.update_actors = True
        self.prop_legend.update_widget(self)
    ...
    def split_multipart(self):
        ...
        for uid in self.selected_uids:
            if isinstance(collection.get_uid_vtk_obj(uid), (PolyLine, TriSurf)):
                ...
                self.TextTerminal.appendPlainText("removed multi-part with uid: " + uid)
    ...
    def create_empty(self):
        ...
        and update the widget."""  # ___________________________________________________________ UPDATE THIS TO ALLOW SORTING BY PROPERTY NAME
    ...
    def save_project(self):
        ...
        prgs_bar = progress_dialog(max_value=self.geol_coll.df.shape[0], title_txt="Save geology",
                                   label_txt="Saving geological objects...", cancel_txt=None, parent=self)
        ...
        prgs_bar = progress_dialog(max_value=self.dom_coll.df.shape[0], title_txt="Save DOM",
                                   label_txt="Saving DOM objects...", cancel_txt=None, parent=self)
        for uid in self.dom_coll.df['uid'].to_list():
            if self.dom_coll.df.loc[self.dom_coll.df['uid'] == uid, 'dom_type'].values[0] == "DEM":
                ...
                pl_writer.Write()
                prgs_bar.add_one()
            elif self.dom_coll.df.loc[self.dom_coll.df['uid'] == uid, 'dom_type'].values[
        ...
        prgs_bar = progress_dialog(max_value=self.image_coll.df.shape[0], title_txt="Save image",
                                   label_txt="Saving image objects...", cancel_txt=None, parent=self)
        for uid in self.image_coll.df['uid'].to_list():
            if self.image_coll.df.loc[self.image_coll.df['uid'] == uid, 'image_type'].values[0] in ["MapImage",
                                                                                                    "XsImage",
                                                                                                    "TSDomImage"]:
        ...
        prgs_bar = progress_dialog(max_value=self.mesh3d_coll.df.shape[0], title_txt="Save 3D mesh",
                                   label_txt="Saving 3D mesh objects...", cancel_txt=None, parent=self)
        ...
        prgs_bar = progress_dialog(max_value=self.boundary_coll.df.shape[0], title_txt="Save boundary",
                                   label_txt="Saving boundary objects...", cancel_txt=None, parent=self)
        ...
        # self.boundary_coll.df[out_cols].to_csv(out_dir_name + '/boundary_table.csv', encoding='utf-8', index=False)
        prgs_bar = progress_dialog(max_value=self.well_coll.df.shape[0], title_txt="Save wells",
        ...
        # self.geol_coll.df[out_cols].to_csv(out_dir_name + '/geological_table.csv', encoding='utf-8', index=False)
        prgs_bar = progress_dialog(max_value=self.fluids_coll.df.shape[0], title_txt="Save fluids",
        ...
        # self.geol_coll.df[out_cols].to_csv(out_dir_name + '/geological_table.csv', encoding='utf-8', index=False)
        prgs_bar = progress_dialog(max_value=self.backgrounds_coll.df.shape[0], title_txt="Save Backgrounds",
    ...
    def open_project(self):
        ...
        if self.geol_coll.get_number_of_entities() > 0:
            confirm_new = QMessageBox.question(self,
                                               'Open Project',
        ...
        To open a different one, edit the project file."""  # ______________________ IN THE FUTURE an option to open a specific revision could be added
        ...
        """In the following it is still possible to open old projects with metadata stored
         ...
         from the class definitions specifies the type of each column."""  # __________ CONSIDER REMOVING THE POSSIBILITY TO OPEN OLD PROJECTS WITH CSV TABLES, THAT WILL CAUSE ERRORS IN CASE OF LISTS
        ...
        if os.path.isfile((in_dir_name + '/xsection_table.csv')) or os.path.isfile(
            ...
            if os.path.isfile((in_dir_name + '/xsection_table.json')):
                ...
                self.xsect_coll.df = pd_read_csv(in_dir_name + '/xsection_table.csv', encoding='utf-8',
                                                 dtype=XSectionCollection.section_type_dict, keep_default_na=False)
        ...
        if os.path.isfile((in_dir_name + '/dom_table.csv')) or os.path.isfile((in_dir_name + '/dom_table.json')):
            ...
            if os.path.isfile((in_dir_name + '/dom_table.json')):
                ...
                self.dom_coll.df = pd_read_csv(in_dir_name + '/dom_table.csv', encoding='utf-8',
                                               dtype=DomCollection.dom_entity_type_dict, keep_default_na=False)
            prgs_bar = progress_dialog(max_value=self.dom_coll.df.shape[0], title_txt="Open DOM",
                                       label_txt="Opening DOM objects...", cancel_txt=None, parent=self)
            for uid in self.dom_coll.df['uid'].to_list():
        ...
        if os.path.isfile((in_dir_name + '/image_table.csv')) or os.path.isfile((in_dir_name + '/image_table.json')):
            ...
            if os.path.isfile((in_dir_name + '/image_table.json')):
                ...
                self.image_coll.df = pd_read_csv(in_dir_name + '/image_table.csv', encoding='utf-8',
                                                 dtype=ImageCollection.image_entity_type_dict, keep_default_na=False)
            prgs_bar = progress_dialog(max_value=self.image_coll.df.shape[0], title_txt="Open image",
                                       label_txt="Opening image objects...", cancel_txt=None, parent=self)
            for uid in self.image_coll.df['uid'].to_list():
                if self.image_coll.df.loc[self.image_coll.df['uid'] == uid, 'image_type'].values[0] in ["MapImage",
                                                                                                        "TSDomImage"]:
                    ...
                    if not os.path.isfile((in_dir_name + "/" + uid + ".vti")):
                        print("error: missing image file")
                        return
                    vtk_object = XsImage(parent=self, x_section_uid=
                    self.image_coll.df.loc[self.image_coll.df['uid'] == uid, 'x_section'].values[0])
                    im_reader = vtkXMLImageDataReader()
                    im_reader.SetFileName(in_dir_name + "/" + uid + ".vti")
                    im_reader.Update()
                    vtk_object.ShallowCopy(im_reader.GetOutput())
        ...
        if os.path.isfile((in_dir_name + '/mesh3d_table.csv')) or os.path.isfile((in_dir_name + '/mesh3d_table.json')):
            ...
            if os.path.isfile((in_dir_name + '/mesh3d_table.json')):
                ...
                self.mesh3d_coll.df = pd_read_csv(in_dir_name + '/mesh3d_table.csv', encoding='utf-8',
                                                  dtype=Mesh3DCollection.mesh3d_entity_type_dict, keep_default_na=False)
            prgs_bar = progress_dialog(max_value=self.mesh3d_coll.df.shape[0], title_txt="Open 3D mesh",
                                       label_txt="Opening 3D mesh objects...", cancel_txt=None, parent=self)
            for uid in self.mesh3d_coll.df['uid'].to_list():
                if self.mesh3d_coll.df.loc[self.mesh3d_coll.df['uid'] == uid, 'mesh3d_type'].values[0] in ["Voxet"]:
                    if not os.path.isfile((in_dir_name + "/" + uid + ".vti")):
                    ...
                    if not os.path.isfile((in_dir_name + "/" + uid + ".vti")):
                        ...
                        return
                    vtk_object = XsVoxet(
                        x_section_uid=self.mesh3d_coll.df.loc[self.mesh3d_coll.df['uid'] == uid, 'x_section'].values[0],
                        parent=self)
                    im_reader = vtkXMLImageDataReader()
                    im_reader.SetFileName(in_dir_name + "/" + uid + ".vti")
                    im_reader.Update()
        ...
        if os.path.isfile((in_dir_name + '/boundary_table.csv')) or os.path.isfile(
            ...
            if os.path.isfile((in_dir_name + '/boundary_table.json')):
                ...
                self.boundary_coll.df = pd_read_csv(in_dir_name + '/boundary_table.csv', encoding='utf-8',
                                                    dtype=BoundaryCollection.boundary_entity_type_dict,
                                                    keep_default_na=False)
            prgs_bar = progress_dialog(max_value=self.boundary_coll.df.shape[0], title_txt="Open boundary",
                                       label_txt="Opening boundary objects...", cancel_txt=None, parent=self)
            for uid in self.boundary_coll.df['uid'].to_list():
        ...
        if os.path.isfile((in_dir_name + '/well_table.csv')) or os.path.isfile((in_dir_name + '/well_table.json')):
            ...
            if os.path.isfile((in_dir_name + '/well_table.json')):
                ...
                self.well_coll.df = pd_read_csv(in_dir_name + '/well_table.csv', encoding='utf-8',
                                                dtype=WellCollection.well_entity_type_dict, keep_default_na=False)
            prgs_bar = progress_dialog(max_value=self.well_coll.df.shape[0], title_txt="Open wells",
        ...
        if os.path.isfile((in_dir_name + '/geological_table.csv')) or os.path.isfile(
            ...
            if os.path.isfile((in_dir_name + '/geological_table.json')):
                ...
                self.geol_coll.df = pd_read_csv(in_dir_name + '/geological_table.csv', encoding='utf-8',
                                                dtype=GeologicalCollection.geological_entity_type_dict,
                                                keep_default_na=False)
            prgs_bar = progress_dialog(max_value=self.geol_coll.df.shape[0], title_txt="Open geology",
                                       label_txt="Opening geological objects...", cancel_txt=None, parent=self)
            for uid in self.geol_coll.df['uid'].to_list():
                if not os.path.isfile((in_dir_name + "/" + uid + ".vtp")):
                    print("error: missing VTK file")
                    return
        ...
        if os.path.isfile((in_dir_name + '/fluids_table.csv')) or os.path.isfile((in_dir_name + '/fluids_table.json')):
            ...
            if os.path.isfile((in_dir_name + '/fluids_table.json')):
                ...
                self.fluids_coll.df = pd_read_csv(in_dir_name + '/fluids_table.csv', encoding='utf-8',
                                                  dtype=FluidsCollection.fluid_entity_type_dict, keep_default_na=False)
            prgs_bar = progress_dialog(max_value=self.fluids_coll.df.shape[0], title_txt="Open fluids",
        ...
        if os.path.isfile((in_dir_name + '/backgrounds_table.csv')) or os.path.isfile(
            ...
            if os.path.isfile((in_dir_name + '/backgrounds_table.json')):
                ...
                self.backgrounds_coll.df = pd_read_csv(in_dir_name + '/backgrounds_table.csv', encoding='utf-8',
                                                       dtype=FluidsCollection.fluid_entity_type_dict,
                                                       keep_default_na=False)
            prgs_bar = progress_dialog(max_value=self.backgrounds_coll.df.shape[0], title_txt="Open fluids",
                                       label_txt="Opening fluid objects...", cancel_txt=None, parent=self)
            for uid in self.backgrounds_coll.df['uid'].to_list():
                if not os.path.isfile((in_dir_name + "/" + uid + ".vtp")):
                    print("error: missing VTK file")
    ...
    def import_gocad(self):
        ...
        if in_file_name:
            ...
            self.prop_legend.update_widget(parent=self)
            self.TextTerminal.appendPlainText('imported Gocad file: ' + in_file_name)
    ...
    def import_PC(self):
        """Import point cloud data. File extension dependent (.txt, .xyz, .las) -> Ui_ImportOptionsWindow ui to
        preview the data (similar to stereonet)"""
        default_attr_list = ['As is',
                             'X', 'Y', 'Z',
                             'Red', 'Green', 'Blue',
                             'Intensity',
                             'Nx', 'Ny', 'Nz',
        ...
        if args:
            ...
            pc2vtk(in_file_name=in_file_name,
                   col_names=col_names,
                   row_range=row_range,
                   usecols=index_list,
                   delimiter=delimiter,
                   self=origin,
                   header_row=0)
            self.TextTerminal.appendPlainText('imported point cloud file: ' + in_file_name)

    def import_SHP(self):
        # ___________________________________________________ UPDATE IN shp2vtk.py OR DUPLICATE THIS TO IMPORT SHP GEOLOGY OR BOUNDARY
        ...
        if in_file_name and coll:
            shp2vtk(self=self, in_file_name=in_file_name, collection=coll)
            self.TextTerminal.appendPlainText('imported SHP file: ' + in_file_name)

    def import_DEM(self):
        """Import DEM file and update DEM collection."""
        #   <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<       >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> see what happens for surfaces as Geotiffs
        """Select and open input file"""
        ...
        if in_file_name:
            ...
            self.TextTerminal.appendPlainText('imported DEM file: ' + in_file_name)
    ...
    def import_mapimage(self):
        ...
        if in_file_name:
            ...
            self.TextTerminal.appendPlainText('imported image file: ' + in_file_name)
    ...
    def import_xsimage(self):
        ...
        if in_file_name:
            """Select the Xsection"""
            if self.xsect_coll.get_uids():
                x_section_name = input_combo_dialog(parent=None,
                                                    title="Xsection",
    ...
    def import_welldata(self):
        ...
        if path:
            ...
            self.prop_legend.update_widget(parent=self)
            self.TextTerminal.appendPlainText('imported well from path: ' + path)
        else:
            return
    ...
    def import_SEGY(
        ...
        """Select and open input file"""
        in_file_name = open_file_dialog(parent=self,
                                        caption='Import SEGY from file',
                                        filter="SEGY (*.sgy *.segy)")
        if in_file_name:
            segy2vtk(self=self, in_file_name=in_file_name)
            self.TextTerminal.appendPlainText('imported SEGY file: ' + in_file_name)
    ...
    def export_cad(self):
        ...
        cad_format = input_combo_dialog(parent=self, title="CAD format",
                                        label="Choose CAD format",
                                        choice_list=["DXF", "GOCAD", "GLTF",
                                                     "CESIUM", "OBJ", "PLY",
                                                     "STL", "STL with 1m dilation", "LandXML"])
        ...
        if cad_format == "DXF":
            ...
            # _______________________________________________________NEED TO IMPLEMENT GOCAD EXPORT<<<<<<<<<<<<<<<<<<<<<<<<
        ...
        self.TextTerminal.appendPlainText('files exported in format: ' + cad_format)

    def export_vtk(self):
        """Export selected objects as vtk files"""
        if not self.selected_uids:
            return
        out_dir_name = save_file_dialog(parent=self, caption="Select save directory.", directory=True)
        for uid in self.selected_uids:  # [gabriele] this could be generalized with a helper function
            if self.shown_table == "tabGeology":
                entity = self.geol_coll.get_uid_vtk_obj(uid)
                pd_writer = vtkXMLPolyDataWriter()
                pd_writer.SetFileName(f'{out_dir_name}/{uid}.vtp')
                pd_writer.SetInputData(entity)
                pd_writer.Write()
                border = entity.get_clean_boundary()
                pd_writer = vtkXMLPolyDataWriter()
                pd_writer.SetFileName(f'{out_dir_name}/{uid}_border.vtp')
                ...
                entity = self.dom_coll.get_uid_vtk_obj(uid)
                temp = vtkPolyData()
                temp.ShallowCopy(entity)  # I hate this <<<<<<<<<<<<<<<<<IS THIS NECESSARY ? DO WE NEED VTP INSTEAD OF VTS?
                pd_writer = vtkXMLPolyDataWriter()
                pd_writer.SetFileName(f'{out_dir_name}/{uid}.vtp')
                ...
                pd_writer.SetInputData(entity)
                pd_writer.Write()
            self.TextTerminal.appendPlainText('files exported in VTK format')

    # Everything here is very bad, but I am short on time
    def export_csv(self):
        """ADD COMMENTS <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<NOT CLEAR WHAT IS THIS<<<<<<<<<<<<<<<<<<<<<<<<< """
        if not self.selected_uids:
            return
        else:
            out_dir_name = save_file_dialog(parent=self, caption="Select save directory.", directory=True)
                        ...
                        for key in entity.point_data_keys:
                            ...
                            if data.ndim == 1:
                                ...
                                for i in range(data.ndim):
                                    df[f'{key}_{i}'] = data[:, i]
                    df.to_csv(f'{out_dir_name}/{uid}.csv')
                else:
