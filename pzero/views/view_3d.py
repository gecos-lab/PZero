...
class View3D(ViewVTK):
    ...
    def on_property_toggled(self, collection_name=None, uid=None, prop_text=None):
        try:
            ...
            prefix_map = {
                'mesh3d_coll': 'Mesh', 'geol_coll': 'Geological', 'xsect_coll': 'Cross-section',
                'boundary_coll': 'Boundary', 'dom_coll': 'DOM', 'image_coll': 'Image',
                'well_coll': 'Well', 'fluid_coll': 'Fluid', 'backgrnd_coll': 'Background'
            }
            ...
            for axis in ['X', 'Y', 'Z']:
                ...
                if hasattr(self, 'slice_actors') and slice_uid in self.slice_actors:
                    self._rebuild_slice_actor(labeled_name, axis, enforced_prop=prop_text)
            # Update any existing grid slices for this entity (multi-slice)
            if hasattr(self, 'slice_actors'):
                for sid in [uid_ for uid_ in list(self.slice_actors.keys()) if uid_.startswith(f"{labeled_name}_") and "_grid_" in uid_]:
    ...
    def _rebuild_slice_actor(self, labeled_name, axis, enforced_prop=None):
        try:
            ...
            if axis == 'X' and b[1] > b[0]:
                ...
                norm = 0.5
            position = {'X': b[0] + norm * (b[1] - b[0]), 'Y': b[2] + norm * (b[3] - b[2]), 'Z': b[4] + norm * (b[5] - b[4])}[axis]
            normal = {'X': [1,0,0], 'Y': [0,1,0], 'Z': [0,0,1]}[axis]
            origin_vec = {'X': [position, 0, 0], 'Y': [0, position, 0], 'Z': [0, 0, position]}[axis]
            ...
            if prop_text is None:
                if hasattr(self, 'slice_prop_by_entity') and labeled_name in self.slice_prop_by_entity:
                    prop_text = self.slice_prop_by_entity[labeled_name]
                elif main_uid is not None:
                    prop_text = self.actors_df.loc[self.actors_df['uid'] == main_uid, 'show_property'].values[0]
            # Style
            scalar_array = None
            cmap = None
            color_RGB = None
            if not prop_text or prop_text == 'none':
                ...
                if hasattr(self.parent, 'prop_legend_df') and self.parent.prop_legend_df is not None:
                    row = self.parent.prop_legend_df[self.parent.prop_legend_df['property_name'] == prop_text]
                    if not row.empty:
    ...
    def _rebuild_grid_slice_actor(self, slice_uid, enforced_prop=None):
        try:
            ...
            prop_text = enforced_prop
            if prop_text is None:
                if hasattr(self, 'slice_prop_by_entity') and entity_name in self.slice_prop_by_entity:
    ...
    def show_mesh_slicer_dialog(self):
            ...
            single_mode_radio.toggled.connect(lambda checked: stack.setCurrentIndex(0) if checked else None)
            multi_mode_radio.toggled.connect(lambda checked: stack.setCurrentIndex(1) if checked else None)
            ...
            def toggle_multi_manipulation(state):
                    ...
                    try:
                        ...
                        for slice_id in existing_slice_ids:
                            ...
                            if slice_type == 'X' and bounds[1] > bounds[0]:
                                normalized_pos = (origin[0] - bounds[0]) / (bounds[1] - bounds[0])
                                ...
                                normalized_pos = (origin[1] - bounds[2]) / (bounds[3] - bounds[2])
                            ...
                            def create_slice_update_callback(target_slice_id, target_entity_name, target_slice_type):
                                def slice_update_callback(normal, widget_origin):
                                    ...
                                    current_bounds = self.get_entity_by_name(target_entity_name).bounds
                                    new_normalized_pos = 0.5 # Default fallback
                                    if target_slice_type == 'X' and current_bounds[1] > current_bounds[0]:
                                        new_normalized_pos = (widget_origin[0] - current_bounds[0]) / (current_bounds[1] - current_bounds[0])
                                    elif target_slice_type == 'Y' and current_bounds[3] > current_bounds[2]:
                                        new_normalized_pos = (widget_origin[1] - current_bounds[2]) / (current_bounds[3] - current_bounds[2])
                            ...
                            widget = self.create_single_plane_widget(
                                slice_type, normalized_pos, bounds,
                                create_slice_update_callback(slice_id, entity_name, slice_type) # Pass slice_id to callback context
                            )
                ...
                try:
                    ...
                    prop_text_sync = None
                    if hasattr(self, 'slice_prop_by_entity') and entity_name_sync in self.slice_prop_by_entity:
                        prop_text_sync = self.slice_prop_by_entity[entity_name_sync]
                    elif main_uid_sync is not None:
                        prop_text_sync = self.actors_df.loc[self.actors_df['uid'] == main_uid_sync, 'show_property'].values[0]
                    for sid in [uid for uid in list(self.slice_actors.keys()) if uid.startswith(f"{entity_name_sync}_{slice_type_sync}_grid_")]:
            ...
            def update_slice_visualization(entity_name, slice_type, normalized_position, fast_update=False, specific_slice_id=None):
                ...
                slice_uid = specific_slice_id if specific_slice_id else f"{entity_name}_{slice_type}"
                                ...
                                if not current_prop or current_prop == 'none':
                                    ...
                                    if hasattr(self.parent, 'prop_legend_df') and self.parent.prop_legend_df is not None:
                                        row = self.parent.prop_legend_df[
                                            self.parent.prop_legend_df['property_name'] == current_prop
                                        ]
                                        if not row.empty:
                                            cmap = row['colormap'].iloc[0]
                                else:
                ...
                try:
                    if is_checked:
                        ...
                        if main_uid:
                            ...
                            coll_name = self.actors_df.loc[self.actors_df['uid'] == main_uid, 'collection'].values[0]
            ...
            def on_check_changed(check_box, slice_type):
                ...
                if slice_uid in self.slice_actors:
                    ...
                    try:
                        ...
                        main_uid = self.get_entity_uid_by_name(entity_name)
                        if main_uid is not None:
                            collection = getattr(self.parent, self.actors_df.loc[self.actors_df['uid'] == main_uid, 'collection'].values[0])
                            labeled_name = None
                            # Rebuild labeled name using same logic as in on_property_toggled
                            for coll_name, prefix in [('mesh3d_coll','Mesh'),('geol_coll','Geological'),('xsect_coll','Cross-section'),('boundary_coll','Boundary'),('dom_coll','DOM'),('image_coll','Image'),('well_coll','Well'),('fluid_coll','Fluid'),('backgrnd_coll','Background')]:
                                if collection.collection_name == coll_name:
                                    labeled_name = f"{prefix}: {collection.get_uid_name(main_uid)}"
                                    break
                            if labeled_name:
                                ...
                                if hasattr(self, 'slice_prop_by_entity') and labeled_name in self.slice_prop_by_entity:
                                    ...
                                    prop_text = self.actors_df.loc[self.actors_df['uid'] == main_uid, 'show_property'].values[0]
                                self._rebuild_slice_actor(labeled_name, slice_type, enforced_prop=prop_text)
                    except Exception:
                        pass
            ...
            def on_mode_switch(mode_index):
                ...
                if mode_index == 0:  # Single slice mode
                    ...
                    entity_name = entity_combo.currentText()
                    if entity_name:
                        for slice_type, check_box in [('X', u_slice_check), ('Y', v_slice_check), ('Z', w_slice_check)]:
                            slice_uid = f"{entity_name}_{slice_type}"
            ...
            single_mode_radio.toggled.connect(lambda checked: on_mode_switch(0) if checked else None)
            multi_mode_radio.toggled.connect(lambda checked: on_mode_switch(1) if checked else None)
    ...
    def _legend_color_for_uid(self, uid):
            ...
            try:
                coll_name = self.actors_df.loc[self.actors_df['uid'] == uid, 'collection'].values[0]
                collection = getattr(self.parent, coll_name)
                    ...
                    if entity:
                        ...
                        try:
                            ...
                            if slice_data.n_points > 0:
                                ...
                                color_RGB = None
                                try:
                                    main_uid = self.get_entity_uid_by_name(entity_name_from_slice)
                                    current_prop = None
                                    if main_uid is not None:
                                        current_prop = self.actors_df.loc[
                                            self.actors_df['uid'] == main_uid, 'show_property'
                                    ...
                                    if not current_prop or current_prop == 'none':
                                        ...
                                        scalar_array = slice_data.points[:, idx]
                                        if hasattr(self.parent, 'prop_legend_df') and self.parent.prop_legend_df is not None:
                                            row = self.parent.prop_legend_df[self.parent.prop_legend_df['property_name'] == current_prop]
                                            if not row.empty:
                                                cmap = row['colormap'].iloc[0]
                                    else:
                                        if current_prop in slice_data.array_names:
