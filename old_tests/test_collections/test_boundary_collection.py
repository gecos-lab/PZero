...
class TestBoundaryCollection:
    ...
    def test_add_entity_from_dict(self):
        ...
        assert (
            self.boundary_coll_istance.get_number_of_entities() == 1
    ...
    def test_remove_entity(self):
        ...
        assert (
            self.boundary_coll_istance.get_number_of_entities() == 0
    ...
    def test_replace_vtk(self):
        ...
        assert (
            self.boundary_coll_istance.get_number_of_entities() == 1
            and self.boundary_coll_istance.get_uid_vtk_obj(self.entity_dict["uid"])
            == self.test_vtk_obj2
        )
    ...
    def test_get_topological_type_uids(self):
        ...
        assert [self.entity_dict["uid"]] == boundary_coll_istance.get_topology_uids(
            self.entity_dict["topology"]
        )
        assert [
            self.boundary_entity_dict2["uid"]
        ] == boundary_coll_istance.get_topology_uids(
            self.boundary_entity_dict2["topology"]
        )
        assert [
            self.boundary_entity_dict3["uid"]
        ] == boundary_coll_istance.get_topology_uids(
            self.boundary_entity_dict3["topology"]
        )
    ...
    def test_get_same_topological_type_uids(self):
        ...
        assert [
            self.boundary_entity_dict3["uid"],
            self.boundary_entity_dict4["uid"],
        ] == boundary_coll_istance.get_topology_uids(
            self.boundary_entity_dict3["topology"]
        )
    ...
    def test_get_uid_names(self):
        ...
        assert self.entity_dict["name"] == boundary_coll_istance.get_uid_name(
            self.entity_dict["uid"]
        )
        assert self.boundary_entity_dict2["name"] == boundary_coll_istance.get_uid_name(
            self.boundary_entity_dict2["uid"]
        )
        assert self.boundary_entity_dict3["name"] == boundary_coll_istance.get_uid_name(
            self.boundary_entity_dict3["uid"]
        )
    ...
    def test_set_uid_names(self):
        ...
        assert new_name2 == boundary_coll_istance.get_uid_name(
            self.boundary_entity_dict2["uid"]
        )
        assert new_name3 == boundary_coll_istance.get_uid_name(
            self.boundary_entity_dict3["uid"]
        )
    ...
    def test_get_uid_topological_type(self):
        ...
        assert self.entity_dict["topology"] == boundary_coll_istance.get_uid_topology(
            self.entity_dict["uid"]
        )
        assert self.boundary_entity_dict2[
            "topology"
        ] == boundary_coll_istance.get_uid_topology(self.boundary_entity_dict2["uid"])
        assert self.boundary_entity_dict3[
            "topology"
        ] == boundary_coll_istance.get_uid_topology(self.boundary_entity_dict3["uid"])
    ...
    def test_set_uid_topological_type(self):
        ...
        boundary_coll_istance.set_uid_topology(
            self.boundary_entity_dict2["uid"], new_top_type2
        )
        boundary_coll_istance.set_uid_topology(
            self.boundary_entity_dict3["uid"], new_top_type3
        )

        assert new_top_type == boundary_coll_istance.get_uid_topology(
            self.entity_dict["uid"]
        )
        assert new_top_type2 == boundary_coll_istance.get_uid_topology(
            self.boundary_entity_dict2["uid"]
        )
        assert new_top_type3 == boundary_coll_istance.get_uid_topology(
            self.boundary_entity_dict3["uid"]
        )
    ...
    def test_get_uid_x_section(self):
        ...
        assert self.entity_dict["x_section"] == boundary_coll_istance.get_uid_x_section(
            self.entity_dict["uid"]
        )
        assert self.boundary_entity_dict2[
            "x_section"
        ] == boundary_coll_istance.get_uid_x_section(self.boundary_entity_dict2["uid"])
        assert self.boundary_entity_dict3[
            "x_section"
        ] == boundary_coll_istance.get_uid_x_section(self.boundary_entity_dict3["uid"])
    ...
    def test_set_uid_x_section(self):
        ...
        boundary_coll_istance.set_uid_x_section(
            self.boundary_entity_dict2["uid"], new_x_sect2
        )
        boundary_coll_istance.set_uid_x_section(
            self.boundary_entity_dict3["uid"], new_x_sect3
        )

        assert new_x_sect == boundary_coll_istance.get_uid_x_section(
            self.entity_dict["uid"]
        )
        assert new_x_sect2 == boundary_coll_istance.get_uid_x_section(
            self.boundary_entity_dict2["uid"]
        )
        assert new_x_sect3 == boundary_coll_istance.get_uid_x_section(
            self.boundary_entity_dict3["uid"]
        )
    ...
    def test_get_uid_vtk_obj(self):
        ...
        assert self.entity_dict["vtk_obj"] == boundary_coll_istance.get_uid_vtk_obj(
            self.entity_dict["uid"]
        )
        assert self.boundary_entity_dict2[
            "vtk_obj"
        ] == boundary_coll_istance.get_uid_vtk_obj(self.boundary_entity_dict2["uid"])
        assert self.boundary_entity_dict3[
            "vtk_obj"
        ] == boundary_coll_istance.get_uid_vtk_obj(self.boundary_entity_dict3["uid"])
    ...
    def test_set_uid_vtk_obj(self):
        ...
        boundary_coll_istance.set_uid_vtk_obj(
            self.boundary_entity_dict2["uid"], new_vtk_obj2
        )
        boundary_coll_istance.set_uid_vtk_obj(
            self.boundary_entity_dict3["uid"], new_vtk_obj3
        )

        assert new_vtk_obj == boundary_coll_istance.get_uid_vtk_obj(
            self.entity_dict["uid"]
        )
        assert new_vtk_obj2 == boundary_coll_istance.get_uid_vtk_obj(
            self.boundary_entity_dict2["uid"]
        )
        assert new_vtk_obj3 == boundary_coll_istance.get_uid_vtk_obj(
            self.boundary_entity_dict3["uid"]
        )
